 ...
 */
@Component
public class FormWorkersProposalSelector implements FormServiceSelector {
    private final String form_kind = "SYY_DQ_LC07";

    @Autowired
    private  FormWorkersProposalService formWorkersProposalService;

    @Override
    public FormService getService(String formkind) {
        if (form_kind.equals(formkind)){
            return formWorkersProposalService;
        }
        return null;
    }
