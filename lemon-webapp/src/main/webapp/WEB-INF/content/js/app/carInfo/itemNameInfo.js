...
define(function (require, exports, module) {
    ...
    loadData();
    $("#itemProjectQueryBtn").click(function () {
        loadData();
    });

    //加载数据
    function loadData() {
        var tpl = require("text!app/tpl/carInfo/ItemProjectTpl.html");
   ...
   var callb= function callback() {
        $("#addItemProjectInfoForm").validate({
            rules: {
                itemName:{
                    required:true
                }
            ...
            submitHandler: function (form) {
                ...
                var options = {
                    ...
                    type: 'post',
                    data: $("#addItemProjectInfoForm").serializeObject(),
   ...
   $("body").delegate("#cancelModal", "click", function () {
    ...
    function deleteMenu(selectids) {
        if(confirm("确认执行删除操作吗？")){
            var options = {
                url: "ajax_deleteItem?t=" + new Date().getMilliseconds() + "&ids=" + selectids,
                type: 'get',
                success: function (data) {
                    if (data == "success") {
                        alert("删除成功");
