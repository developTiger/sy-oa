...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "ajax_kg04_lotTpl1";
        var tpl = require("text!app/tpl/delay/syy_kg_lc04_kgTpl1.html");
...
//*********************************************
    ...
    function ajax_lotProjectNO(selectids, sid, allContent) {
        var options = {
            url: "ajax_kgk_approve?t=" + new Date().getMilliseconds() + "&ids=" + selectids + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
    ...
    function ajax_lotProjectNOONE(prono, sid, allContent) {
        var options = {
            url: "ajax_kgk_approve_pro?t=" + new Date().getMilliseconds() + "&forno=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("否决成功");
...
//群审批同意
    ...
    function ajax_lotProjectYES(prono, sid, allContent) {
        var options = {
            url: "ajax_kgk_approve_yes?t=" + new Date().getMilliseconds() + "&ids=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
    ...
    function ajax_lotProjectYESONE(prono, sid, allContent) {
        var options = {
            url: "ajax_kgk_approve_proyes?t=" + new Date().getMilliseconds() + "&prono=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
...
//多选获取fenguan
    ...
    function ajax_lotProjectYES(prono, sid, allContent) {
        var options = {
            url: "ajax_approval_approves?t=" + new Date().getMilliseconds() + "&ids=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
