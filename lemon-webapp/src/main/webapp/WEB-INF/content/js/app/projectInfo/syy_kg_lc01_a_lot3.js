...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "ajax_kg01_lotTpl3";
        var tpl = require("text!app/tpl/projectInfo/syy_kg_lc01_lotTpl3.html");
...
//群审批否决上一级
    $("body").delegate("#lotReject3", "click", function () {
        var chk_value = [];
    ...
    function ajax_lotProjectNOONE(prono, sid, allContent) {
        var options = {
            url: "ajax_lotProjectNOONE3?t=" + new Date().getMilliseconds() + "&forno=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("否决成功");
...
//群审批同意
    $("body").delegate("#lotApprove3", "click", function () {
        var chk_value = [];
        $('.checkBtn:checked').each(function () {
            chk_value.push($(this).val());
        });
    ...
    function ajax_lotProjectYESONE(prono, sid, allContent) {
        var options = {
            url: "ajax_lotProjectYESONE3?t=" + new Date().getMilliseconds() + "&prono=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
