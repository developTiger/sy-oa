...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "syy_kg06_plTpl2";
        var tpl = require("text!app/tpl/delivers/syy_kg_lc06_lotTpl2.html");
...
//群审批打回上一级
    $("body").delegate("#goBack", "click", function () {
        ...
        ajax_lotProjectNO(chk_value.toString(),allContent,proficientOpinion);
    });
    function ajax_lotProjectNO(prono,allContent,proficientOpinion) {
        var options = {
            url: "syy_kg_lc06_approve2?t=" + new Date().getMilliseconds()+"&proficientOpinion="+encodeURI(encodeURI(proficientOpinion)) + "&ids=" + prono +"&allContent=" +encodeURI(encodeURI(allContent))+"&back=back",
            type: 'get',
...
//群审批同意
    $("body").delegate("#lotApprove", "click", function () {
        ...
        ajax_lotProjectYES(chk_value.toString(),allContent,proficientOpinion);
    });

    function ajax_lotProjectYES(prono,allContent,proficientOpinion) {
        var options = {
            url: "syy_kg_lc06_approve2?t=" + new Date().getMilliseconds()+"&proficientOpinion="+encodeURI(encodeURI(proficientOpinion)) + "&ids=" + prono +"&allContent=" +encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if(data=="success"){
                    alert("签核成功");
