...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "ajax_kg01_lotTpl5_kgk";
        var tpl = require("text!app/tpl/projectInfo/syy_kg_lc01_lotTpl5_kgk.html");
    ...
    //群审批否决上一级
    $("body").delegate("#lotReject5_kgk", "click", function () {
        var chk_value = [];
    ...
    function ajax_lotProjectNO(selectids, sid, allContent) {
        var options = {
            url: "ajax_lotProjectNO5_kgk?t=" + new Date().getMilliseconds() + "&ids=" + selectids + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
...
//群审批同意
    $("body").delegate("#lotApprove5_kgk", "click", function () {
        var chk_value = [];
        $('.checkBtn:checked').each(function () {
            chk_value.push($(this).val());
        });
    ...
    function ajax_lotProjectYES(prono, sid, allContent) {
        var options = {
            url: "ajax_lotProjectYES5_kgk?t=" + new Date().getMilliseconds() + "&ids=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
