...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "ajax_kg05_lotTpl2";
        var tpl = require("text!app/tpl/projectInfo/syy_kg_lc05_kgTpl2.html");
...
//群审批打回上一级
    ...
    function ajax_lotProjectNO(selectids, sid, allContent) {
        var options = {
            url: "ajax_approval_reject2?t=" + new Date().getMilliseconds() + "&ids=" + selectids + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
...
//群审批同意
    ...
    function ajax_lotProjectYES(prono, sid, allContent) {
        var options = {
            url: "ajax_approval_approve1_2?t=" + new Date().getMilliseconds() + "&ids=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
    ...
    function ajax_lotProjectNOONE(prono, sid, allContent) {
        var options = {
            url: "ajax_kygl01_approve_pro?t=" + new Date().getMilliseconds() + "&forno=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("打回成功");
    ...
    function ajax_lotProjectYESONE(prono, sid, allContent) {
        var options = {
            url: "ajax_kygl01_approve_proyes?t=" + new Date().getMilliseconds() + "&prono=" + prono + "&empids=" + sid + "&allContent=" + encodeURI(encodeURI(allContent)),
            type: 'get',
            success: function (data) {
                if (data == "success") {
                    alert("签核成功");
