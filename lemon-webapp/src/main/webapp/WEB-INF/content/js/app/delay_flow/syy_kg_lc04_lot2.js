...
define(function (require, exports, module) {
    ...
    function loadData() {
        var url = "syy_kg04_plTpl2";
        var tpl  = require("text!app/tpl/delay/syy_kg_lc04_lotTpl2.html");
...
//群审批打回上一级
    $("body").delegate("#goBack", "click", function () {
        ...
        var leaderWord=$("#leaderWord").val();
        ajax_lotProjectNO(chk_value.toString(),allContent,leaderWord);
    });
    function ajax_lotProjectNO(prono,allContent,leaderWord) {
        var options = {
            url: "syy_kg_lc04_v1_approve_lot?t=" + new Date().getMilliseconds() + "&ids=" + prono +"&content=" +encodeURI(encodeURI(allContent))+"&appvalue=2"+"&leaderWord="+encodeURI(encodeURI(leaderWord)),
            type: 'get',
...
//群审批同意
    $("body").delegate("#lotApprove", "click", function () {
        ...
        var allContent=$("#allContent").val();
        ajax_lotProjectYES(chk_value.toString(),allContent,leaderWord);
    });

    function ajax_lotProjectYES(prono,allContent,leaderWord) {
        var options = {
            url: "syy_kg_lc04_v1_approve_lot?t=" + new Date().getMilliseconds() + "&ids=" + prono +"&content=" +encodeURI(encodeURI(allContent))+"&appvalue=1&leaderWord="+encodeURI(encodeURI(leaderWord)),
            type: 'get',
            success: function (data) {
                if(data=="success"){
                    alert("签核成功");
